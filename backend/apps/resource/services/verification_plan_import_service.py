...
- 核查周期(天)、上次核查时间、下次核查时间
- 核查提前提醒(天)、核查提醒人员、核查方法
"""
import logging
...
EXCEL_COLUMN_MAP = {
    ...
    '型号': 'model_number',
    '核查周期': 'verification_cycle_days',
    '核查周期(天)': 'verification_cycle_days',
    '上次核查时间': 'verification_date',
    '上次核查日期': 'verification_date',
    '下次核查时间': 'next_due_date',
    '下次核查日期': 'next_due_date',
    '核查提前提醒': 'reminder_days',
    '核查提前提醒(天)': 'reminder_days',
    '核查提醒人员': 'reminder_person',
    '核查方法': 'verification_method',
}
...
def parse_excel_rows(file) -> List[dict]:
    ...
    for row_idx, row in enumerate(rows[1:], start=2):
        ...
        if 'verification_date' in col_map and col_map['verification_date'] < len(row):
            verification_date = _parse_date(row[col_map['verification_date']])
        ...
        result.append({
            ...
            'verification_cycle_days': _get_int('verification_cycle_days'),
...
def import_verification_plan_from_rows(
    rows: List[dict],
    created_by_id: Optional[int] = None,
) -> dict:
    """批量导入核查计划。根据设备编号匹配设备，更新 next_verification_date"""
    total = len(rows)
    success = 0
    failed = 0
    errors = []
    ...
    for r in rows:
        ...
        if not code or not next_due:
            failed += 1
            errors.append({'row': row_num, 'code': code, 'message': '设备编号或下次核查时间为空'})
            continue
        ...
        try:
            equip.next_verification_date = next_due
            update_fields = ['next_verification_date', 'update_time']

            if verification_date:
                equip.last_verification_date = verification_date
            ...
            if r.get('verification_cycle_days') is not None:
                equip.verification_cycle_days = r['verification_cycle_days']
                update_fields.append('verification_cycle_days')
            if r.get('status'):
            ...
            for k, v in [
                ('verification_reminder_days', r.get('reminder_days')),
                ('verification_reminder_person', r.get('reminder_person')),
...
def parse_and_import_verification_plan(file, created_by_id=None) -> dict:
    """入口：解析 Excel 并导入核查计划"""
    rows = parse_excel_rows(file)
    if not rows:
        return {'total': 0, 'success': 0, 'failed': 0, 'created_ids': [], 'errors': [{'row': 0, 'code': '', 'message': '未能解析到有效数据行'}]}
    return import_verification_plan_from_rows(rows, created_by_id=created_by_id)
