...
# 与 .env.example 中 DJANGO_DEBUG 对齐；兼容仅写 DEBUG=True 的本地文件（避免 dev-bypass 与 anon_in_debug 全失效）
_debug_raw = os.getenv('DJANGO_DEBUG')
if _debug_raw is None or str(_debug_raw).strip() == '':
    _debug_raw = os.getenv('DEBUG', 'False')
DEBUG = str(_debug_raw).lower() in ('true', '1', 'yes', 'on')
