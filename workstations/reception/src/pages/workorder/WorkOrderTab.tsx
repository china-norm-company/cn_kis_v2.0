/**
 * 工单管理 - 列表、新建、编辑、查看（复刻自度支·物料台 样品发放）
 */
...
function WorkOrderViewContent({
  ...
  return (
    <div className="space-y-4">
      ...
      {detail.schedule_plan && (detail.schedule_plan.rows?.length || detail.schedule_plan.raw) && (
        <Card variant="elevated" className="p-4">
          <h3 className="text-base font-semibold text-slate-800 mb-3">排期计划</h3>
          <p className="text-xs text-slate-500 mb-2">数据来源于执行台项目管理详情页</p>
          {(detail.schedule_plan.overall_start || detail.schedule_plan.overall_end) && (
            <div className="mb-3 flex gap-4 text-sm">
              <span>执行开始日期：{detail.schedule_plan.overall_start ?? '—'}</span>
              <span>执行结束日期：{detail.schedule_plan.overall_end ?? '—'}</span>
            </div>
          )}
          {detail.schedule_plan.rows && detail.schedule_plan.rows.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[320px] text-sm border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50">
                    <th className="text-left px-3 py-2 font-medium">访视时间点</th>
                    {Array.from(
                      { length: Math.max(1, ...detail.schedule_plan.rows.map((r) => r.dates?.length ?? 0)) },
                      (_, i) => (
                        <th key={i} className="text-left px-3 py-2 font-medium">执行日期{i + 1}</th>
                      )
                    )}
                  </tr>
                </thead>
                <tbody>
                  {detail.schedule_plan.rows.map((row, ri) => {
                    const maxDates = Math.max(1, ...detail.schedule_plan!.rows!.map((r) => r.dates?.length ?? 0))
                    return (
                      <tr key={ri} className="border-b border-slate-100">
                        <td className="px-3 py-2">{row.visitPoint}</td>
                        {Array.from({ length: maxDates }, (_, di) => (
                          <td key={di} className="px-3 py-2">{(row.dates ?? [])[di] ?? ''}</td>
                        ))}
                      </tr>
                    )
                  })}
          ...
          ) : detail.schedule_plan.raw ? (
            <pre className="text-sm text-slate-700 whitespace-pre-wrap rounded-lg border border-slate-200 bg-slate-50 p-3">{detail.schedule_plan.raw}</pre>
          ) : null}
        </Card>
      )}
      {(detail.precautions || detail.project_requirements) && (
        <Card variant="elevated" className="p-4">
...
export function WorkOrderTab() {
  ...
  const { data, isLoading, error } = useQuery<{
    ...
    queryKey: ['reception', 'product-distribution', 'workorders', { page, pageSize, keyword }],
    queryFn: async () =>
  ...
  const refresh = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ['reception', 'product-distribution', 'workorders'] })
    ...
    productDistributionApi
      ...
      .then((d: unknown) => {
        ...
        setFormData({
          ...
          project_start_date: String(row.project_start_date ?? ''),
          project_end_date: String(row.project_end_date ?? ''),
          visit_count: Number(row.visit_count) ?? 0,
          researcher: row.researcher != null ? String(row.researcher) : '',
          supervisor: row.supervisor != null ? String(row.supervisor) : '',
          usage_method: row.usage_method != null ? String(row.usage_method) : '',
          usage_frequency: row.usage_frequency != null ? String(row.usage_frequency) : '',
          precautions: row.precautions != null ? String(row.precautions) : '',
          project_requirements: row.project_requirements != null ? String(row.project_requirements) : '',
        })
      })
      .catch(() => {
        setShowEdit(false)
          ...
          {editFormLoading ? (
            ...
            <>
              <WorkOrderForm
                ...
                readOnlyExecutionFields
              />
              {editDetail?.schedule_plan && (
                <div className="pt-4 border-t border-slate-200">
                  <h3 className="text-sm font-semibold text-slate-700 mb-2">排期计划</h3>
                  <p className="text-xs text-slate-500 mb-2">数据来源于执行台项目管理详情页</p>
                  {(editDetail.schedule_plan.overall_start || editDetail.schedule_plan.overall_end) && (
                    <div className="mb-2 flex gap-4 text-sm text-slate-700">
                      <span>执行开始日期：{editDetail.schedule_plan.overall_start ?? '—'}</span>
                      <span>执行结束日期：{editDetail.schedule_plan.overall_end ?? '—'}</span>
                    </div>
                  )}
                  {editDetail.schedule_plan.rows && editDetail.schedule_plan.rows.length > 0 ? (
                    <div className="overflow-x-auto text-sm">
                      <table className="w-full min-w-[280px] border-collapse">
                        <thead>
                          <tr className="border-b border-slate-200 bg-slate-50">
                            <th className="text-left px-2 py-1.5 font-medium">访视时间点</th>
                            {Array.from({ length: Math.max(1, ...editDetail.schedule_plan.rows.map((r) => r.dates?.length ?? 0)) }, (_, i) => (
                              <th key={i} className="text-left px-2 py-1.5 font-medium">执行日期{i + 1}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {editDetail.schedule_plan.rows.map((row, ri) => {
                            const maxDates = Math.max(1, ...editDetail.schedule_plan!.rows!.map((r) => r.dates?.length ?? 0))
                            return (
                              <tr key={ri} className="border-b border-slate-100">
                                <td className="px-2 py-1.5">{row.visitPoint}</td>
                                {Array.from({ length: maxDates }, (_, di) => (
                                  <td key={di} className="px-2 py-1.5">{(row.dates ?? [])[di] ?? ''}</td>
                                ))}
                              </tr>
                            )
                          })}
                  ...
                  ) : editDetail.schedule_plan.raw ? (
                    <pre className="text-sm text-slate-700 whitespace-pre-wrap rounded border border-slate-200 bg-slate-50 p-2">{editDetail.schedule_plan.raw}</pre>
                  ) : null}
                </div>
              )}
              {editDetail && (
