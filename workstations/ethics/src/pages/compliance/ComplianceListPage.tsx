...
export function ComplianceListPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['ethics', 'compliance-checks', page],
    queryFn: () => ethicsApi.getComplianceChecks({ page, page_size: 20 }),
  })
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      <h2 className="text-lg font-semibold text-slate-800 md:text-xl">合规检查</h2>
          ...
          <table className="w-full min-w-[760px]">
            <thead>
              <tr className="border-b border-slate-200 text-left text-xs text-slate-500 uppercase">
                <th className="px-4 py-3">检查编号</th>
                <th className="px-4 py-3">检查类型</th>
                <th className="px-4 py-3">检查范围</th>
                <th className="px-4 py-3">状态</th>
                <th className="px-4 py-3">发现数</th>
            ...
            <tbody>
              {items.map((item: any) => (
                <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="px-4 py-3 text-sm font-medium text-slate-700">{item.check_no || '-'}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{item.check_type_display || item.check_type}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{item.scope || '-'}</td>
                  ...
                  <td className="px-4 py-3 text-sm text-slate-600">{item.finding_count ?? '-'}</td>
