...
export default function TechnicianPage() {
        ...
        if (match) {
          ...
          if (res.code === 200 && qrData) {
            ...
            if (record.today_work_orders?.length === 1) {
              Taro.navigateTo({
                url: `/pages/technician/workorder-detail?id=${record.today_work_orders[0].id}`,
              })
            } else {
  ...
  const navigateToDetail = (woId: number) => {
    Taro.navigateTo({ url: `/pages/technician/workorder-detail?id=${woId}` })
  }
      ...
      <View className="actions">
        ...
        {(isQA(getLocalRoles()) || isManagement(getLocalRoles())) && (
          <View
            ...
            onClick={() => Taro.navigateTo({ url: '/pages/qa-patrol/index' })}
          >
            <Text className="action-icon">📋</Text>
