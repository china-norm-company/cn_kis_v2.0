...
type ProductDetailWithProject = MyProductDetail & {
...
/** 无后端时用于预览的模拟产品详情 */
const MOCK_DETAILS: Record<number, ProductDetailWithProject> = {
  101: {
...
export default function SampleReturnPage() {
  ...
  const [details, setDetails] = useState<Map<number, ProductDetailWithProject>>(new Map())
  ...
  const loadDetails = useCallback(async () => {
    ...
    setLoading(true)
    const map = new Map<number, ProductDetailWithProject>()
      ...
      for (const id of dispensingIds) {
        try {
          ...
          if (res.code === 200 && res.data) {
            const d = res.data as ProductDetailWithProject
            map.set(id, d)
  ...
  const handleExpressChange = (e: { detail: { value: string | number } }) => {
  ...
  const detailList = dispensingIds.map((id) => details.get(id)).filter(Boolean) as ProductDetailWithProject[]
      ...
      <ScrollView scrollY className='sample-return__scroll'>
        <MiniCard>
          ...
          <View className='sample-return__express-section'>
            ...
            <Picker mode='date' value={shipDate} onChange={(e) => setShipDate(String(e.detail.value))}>
              <View className='sample-return__picker'>
