...
& ssh @SshArgs
=======
# Windows：本机端口 -> 远端服务器 PostgreSQL 5432（与 scripts/ssh_tunnel_postgres.sh 等价）
# 用法：在项目根目录 PowerShell 执行 .\scripts\ssh_tunnel_postgres.ps1
# 保持窗口不关；另开终端 cd backend && python manage.py runserver 8001
#
# 本机 5432 被占用时：
#   $env:SSH_TUNNEL_LOCAL_PORT = "5433"; .\scripts\ssh_tunnel_postgres.ps1
# 并在 backend/.env 中设置 DB_PORT=5433
...
$User = if ($env:VOLCENGINE_SSH_USER) { $env:VOLCENGINE_SSH_USER } else { "root" }
$HostName = if ($env:VOLCENGINE_SSH_HOST) { $env:VOLCENGINE_SSH_HOST } else { "118.196.64.48" }
$Key = if ($env:VOLCENGINE_SSH_KEY) { $env:VOLCENGINE_SSH_KEY } else { "" }
$LocalPort = if ($env:SSH_TUNNEL_LOCAL_PORT) { $env:SSH_TUNNEL_LOCAL_PORT } else { "5432" }

Write-Host "隧道: 本机 ${LocalPort} -> ${User}@${HostName} 的 PostgreSQL (远端 5432)"
Write-Host "保持本窗口打开；另开终端: cd backend; python manage.py migrate"
Write-Host "Ctrl+C 退出隧道"
Write-Host ""
...
if ($Key -ne "" -and (Test-Path -LiteralPath $Key)) {
    & ssh @SshOpts -i $Key "${User}@${HostName}"
} else {
    & ssh @SshOpts "${User}@${HostName}"
}
>>>>>>> origin/develop
