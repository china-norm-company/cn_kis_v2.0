#!/bin/bash
# 建立 SSH 隧道：本机 5432 -> 服务器 PostgreSQL 5432
# 使用前请保持 backend/.env 中 DB_HOST=127.0.0.1 DB_PORT=5432（勿设置 USE_SQLITE）
...
echo "隧道: 本机 5432 -> $USER@$HOST 的 PostgreSQL (5432)"
...
if [ -n "$KEY" ] && [ -f "$KEY" ]; then
  exec ssh -N -L 5432:localhost:5432 -i "$KEY" $SSH_OPTS "$USER@$HOST"
else
  exec ssh -N -L 5432:localhost:5432 $SSH_OPTS "$USER@$HOST"
fi