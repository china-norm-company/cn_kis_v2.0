    ...
    <div className={rootClass}>
        ...
        {loading ? (
          <div
            className={clsx(
              ...
              'dark:border-[#3b434e] dark:bg-slate-800 dark:text-slate-400',
            )}
          >
          ...
          data.map((record, index) => (
            <div
              ...
              className={clsx(
                compact
                  ? 'space-y-1.5 rounded-lg border border-slate-200 bg-white p-3'
                  : 'space-y-2 rounded-lg border border-slate-200 bg-white p-4',
                'dark:border-[#3b434e] dark:bg-slate-800',
                'hover:bg-slate-50 transition-colors dark:hover:bg-slate-700/35',
                onRowClick && 'cursor-pointer',
                rowClassName?.(record, index),
              )}
              onClick={() => onRowClick?.(record)}
            >
              {columns.map((col) => {
                ...
                return (
                  ...
                  >
                    <span className="shrink-0 text-xs font-medium text-slate-500 whitespace-nowrap dark:text-slate-400">
                      {title}
                    </span>
      ...
      {renderDesktopStickyCluster ? (
        <div className="hidden min-h-0 flex-1 flex-col overflow-hidden md:flex md:min-h-0">
          ...
          >
            <div className="rounded-b-xl border border-t-0 border-slate-200 bg-white dark:border-[#3b434e]">
              <table className="w-full table-fixed border-separate border-spacing-0">
                {colgroup}
                {tbodySection}
              </table>
        ...
        <div className="cnkis-datatable-desktop-wrap hidden overflow-x-auto rounded-lg border border-slate-200 dark:border-[#3b434e] md:block">
